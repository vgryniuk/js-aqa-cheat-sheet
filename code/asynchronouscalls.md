...
### Arrow functions
**Стрілкові функції** в JavaScript - це синтаксичний цукор для оголошення функцій. Вони зазвичай корисні, коли потрібно передати анонімну функцію або коли функція коротка і проста.
...
// З більшою кількістю параметрів
const func = (param1, param2) => {
  // тіло функції
};

// Повернення значення (однорядковий вираз)
const func = () => expression;
```
Коли використовувати стрілкові функції:
* **Короткі функції**: Коли функція складається з одного виразу або декількох коротких операцій.
* **Колбеки**: Для передачі анонімних функцій у методи чи функції.
* **Контекст `this`**: Стрілкові функції не створюють свого власного контексту `this`, вони унаслідують контекст від зовнішньої функції.

Приклади використання стрілкових функцій:
Робота з масивами:
```
const numbers = [1, 2, 3, 4, 5];

// Перетворення масиву за допомогою стрілкової функції
...
const fetchData = () => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const data = 'Дані з промісу';
      resolve(data);
    }, 2000);
  });
};

// Використання стрілкової функції з промісом
const getData = async () => {
  try {
    const result = await fetchData();
    console.log(result); // Виведе: Дані з промісу через 2 секунди
  } catch (error) {
    console.error(error);
  }
};
